...
| (툴) | **`style_transfer_server.py` + `style_transfer.html` — 인터랙티브 스타일 전이 UI(2026-07-26)** | `test.py`(subprocess), 4-1 리타겟 코드(HTML에 복사) | 콘텐츠 BVH + 스타일 BVH 선택 → 백엔드가 `conda run -n motion_puzzle python test.py`로 Motion Puzzle 실행(~9초) → 결과 BVH를 X Bot 캐릭터로 표시. 파이토치·conda가 필요해 브라우저 단독 불가라 로컬 백엔드(stdlib http.server, 포트 8940). 입력 풀은 `datasets/cmu/test_bvh/*.bvh`. 영상→BVH는 GVHMR(Colab) 단계라 UI 범위 밖(명시). `python style_transfer_server.py`로 실행 |
