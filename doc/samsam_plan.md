...
| 단계 | 작업 | 도구 |
|---|---|---|
| 5-1 | Mixamo 캐릭터 + three.js 뷰어에 스타일 전이 전/후 나란히 재생 | **완료(2026-07-26)** — `samsam_viewer.html`, X Bot 캐릭터 3패널(원본/스타일A/스타일B) |
| 5-2 | 웹에서 before/after를 눈으로 비교 가능하게 | **완료** — 같은 콘텐츠·다른 스타일이 캐릭터로 나란히, 동기 재생·힙 추적 카메라·드래그 회전 |

목표 상태: 웹에서 before/after를 본다. → **달성**(공개 샘플 CMU 스타일 기준). 우리 실촬영 데이터로 교체는 세션 6과 함께.
