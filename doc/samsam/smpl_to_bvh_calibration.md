...
**일부러 뺀 것 — 목→머리(`Neck1`, 42.3°)**: SMPL의 `head` 관절은 목보다 앞쪽에 있고(방향 Z +0.62) CMU `Head`는 거의 수직 위다. 관절 의미 자체가 다르므로 보정하면 머리가 42° 앞으로 숙여진다. 의미가 다른 건 보정이 아니라 왜곡이라 그대로 둔다.
