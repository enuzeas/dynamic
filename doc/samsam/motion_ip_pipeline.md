...
0. **(신규, 최우선) SM-SGE 접근 방식 재검토** — 1-1장 교수 자문 결과, "동작을 넘나드는 개인 불변 성분" 추출이 스타일(저레벨)이 아니라 습관(고레벨) 문제일 수 있다는 지적을 받음. 아래 1~4번 착수 전에 팀 결정 필요:
   ...
   - (d) **(2026-08-06 신규 검토)** 딥러닝 정제 헤드(Triplet Loss) 대신, 문헌상 교차-과제 불변성이 실제로 확인된 **가느다란(thin) 도메인-독립 스칼라**(움직임 vigor·시행 간 변동성 크기 — `papers.md` A4·A9·A11)를 Motion Puzzle 출력 위에 **직접 얹는 해석 가능한 파라미터**로 쓰는 방향. 예: vigor 값으로 재생 속도/움직임 크기를 스케일, variability 값으로 미세한 타이밍 흔들림을 부여. "습관을 학습했다"는 검증 안 된 주장을 피하고 "측정된 숫자를 눈에 보이는 다이얼로 바꿨다"는 가벼운 주장만 하므로, (a)(b)(c)보다 스코프가 작고 남은 기간에 실제로 완성 가능 — 비전공 팀원에게도 설명 가능하다는 점에서 **현업(팀 데모) 사용가능성이 가장 높은 옵션**. 단, 신호가 얇아 사람 수가 늘면 변별력이 떨어지므로 착수 전 `evaluate_icc.py`로 반복 촬영 간 ICC부터 확인할 것(`papers.md` A절 정리 참고).
