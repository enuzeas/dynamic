...
# 일부러 뺀 관절이 둘 있다.
#
# 1) Neck1(15) — SMPL의 head 관절은 목보다 앞쪽에 있고(방향 Z +0.62) CMU Head는 거의 수직 위라
#    관절 의미가 다르다. 보정하면 머리가 42° 앞으로 숙여진다. 의미가 다른 건 보정이 아니라 왜곡이다.
#
# 2) LHipJoint(1)·RHipJoint(6)·LowerBack(11)·LeftShoulder(17)·RightShoulder(24) — Motion Puzzle이
#    쓰는 21관절(SELECTED_31_TO_21)에도, samsam_viewer.html의 CMU_TO_MIXAMO(19개)에도 없다.
#    여기에 보정을 넣으면 자식의 로컬 회전에는 A_parent⁻¹이 들어가는데 정작 부모의 A는 아무도
#    적용하지 않아서, 소비하는 쪽에서 딱 그만큼 틀어진다(2026-09-11: 다리가 23.8°씩 틀어졌다).
#    보정은 "다운스트림이 실제로 읽는 관절"에만 넣어야 한다.
SMPL_REST_DIR = {
...
# 보정은 다운스트림이 실제로 읽는 관절에만. 여기를 어기면 자식에 A_parent⁻¹만 남아 그만큼 틀어진다.
assert set(SMPL_REST_DIR) <= set(SELECTED_31_TO_21.tolist()), \
    f"21관절 밖에 보정을 넣었다: {sorted(set(SMPL_REST_DIR) - set(SELECTED_31_TO_21.tolist()))}"
